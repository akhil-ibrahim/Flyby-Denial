...
Use the following steps as an example input for a test run:

1. **Upload FASTA File**
   - Go to `Input Data → Option 1: Upload a FASTA file`
   - Upload `ALK_mice.faa`
   - This will store **3 protein sequences** in the list

2. **Fetch from NCBI**
   - Go to `Input Data → Option 3: Fetch from NCBI by Organism`
